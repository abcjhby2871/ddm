...
if os.path.isfile("data_exp1.csv"):
    with open("data_exp1.csv", "r", newline="", encoding="utf-8") as file:
...
def save_data(data, filename="data_exp1.csv"):
    """追加实验数据到CSV文件，确保trial编号接着上次继续"""
    keys = ["trial", "snack1", "snack2", "chosen_snack", "reaction_time", "rating_diff", "category", "hint", "correct"]
...
# 第一阶段：评分任务
def phase_1():
    instructions = visual.TextStim(win, text="请对以下零食进行评分，范围从0到10。点击'宁愿不吃'按钮可选择不喜欢该零食。", 
                                   color='black', height=20, pos=(-150, 300))
    ...
    # 创建“宁愿不吃”按钮
    no_eat_button = visual.Rect(win, width=150, height=50, fillColor='red', pos=(0, -250))
    no_eat_text = visual.TextStim(win, text="宁愿不吃", color='white', pos=(0, -250), height=20)

    snack_ratings = {}
    snack_images = [f"{i}.png" for i in range(1, 6)]  # 图片文件名

    for snack in snack_images:
        instructions.draw()

        # 加载当前零食图片
        snack_image = visual.ImageStim(win, image=os.path.join(r"snacks", snack), pos=(0, 100), size=(300, 300))
        snack_image.draw()
        win.flip()
        # 重置Slider和按钮
        rating_slider.reset()
...
def phase_2_train(snack_ratings):
    instructions = visual.TextStim(win, text="本阶段为训练阶段！\n 请选择你更想吃的零食，使用左右箭头键选择。", color='black', height=20, pos=(0, 250))
    ...
    for trial_num, (snack1, snack2, category) in enumerate(snack_pairs[:270]):
        ...
        if hint:
            category_text = {"high": "高值零食对比", "mid": "中值零食对比", "low": "低值零食对比"}[category]
        ...
        snack1_image = visual.ImageStim(win, image=os.path.join(r"snacks", snack1), pos=(-200, 0), size=(200, 200))
        snack2_image = visual.ImageStim(win, image=os.path.join(r"snacks", snack2), pos=(200, 0), size=(200, 200))
...
def phase_2_CV(snack_ratings):
    instructions = visual.TextStim(win, text="请选择你更想吃的零食，使用左右箭头键选择。", color='black', height=20, pos=(0, 250))
